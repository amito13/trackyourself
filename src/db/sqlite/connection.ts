...
export class LocalDatabase {
  ...
  constructor(private readonly connection: SqlConnection, readonly userId: string) {}
  ...
  async initialize(): Promise<void> {
    await this.read(db => db.execAsync('PRAGMA journal_mode = WAL; PRAGMA foreign_keys = ON; PRAGMA busy_timeout = 5000;'));
    await this.write(async db => {
      ...
      const account = await db.getFirstAsync<{ user_id: string }>('SELECT user_id FROM local_account');
