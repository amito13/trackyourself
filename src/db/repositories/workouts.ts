...
async function requireActive(db: SqlConnection, sessionId: string, userId: string) {
  ...
  if (!session || session.status !== 'active')
    throw new Error('An active workout is required');
  return session;
}

async function requireExercise(db: SqlConnection, id: string, userId: string) {
  const exercise = await db.getFirstAsync<SessionExercise>(
    'SELECT * FROM session_exercises WHERE id = ? AND user_id = ? AND deleted_at IS NULL',
    id,
    userId,
  );
...
export class WorkoutRepository {
  ...
  active() {
    return this.local.read((db) =>
      db.getFirstAsync<WorkoutSession>(
        `SELECT * FROM workout_sessions
      WHERE user_id = ? AND status = 'active' ORDER BY started_at, id LIMIT 1`,
        this.local.userId,
      ),
    );
  }
  ...
  today() {
    return this.local.read(async (db) => {
      ...
      const day = await db.getFirstAsync<{ id: string }>(
        ...
        AND p.deleted_at IS NULL AND d.deleted_at IS NULL AND d.day_of_week = ?`,
        this.local.userId,
        weekday,
      );
  ...
  startOrResume() {
    return this.local.write(async (db) => {
      ...
      const active = await db.getFirstAsync<WorkoutSession>(
        `SELECT * FROM workout_sessions
        WHERE user_id = ? AND status = 'active' ORDER BY started_at, id LIMIT 1`,
        userId,
      );
      ...
      if (!plan?.exercises.length)
        throw new Error('Today’s plan has no available exercises');
      const sessionId = this.runtime.uuid();
      await insertRecord(db, 'workout_sessions', {
        id: sessionId,
        user_id: userId,
        workout_day_id: day.id,
        workout_date: localDate(date),
        title: plan.bodyParts.join(' + '),
        body_parts: JSON.stringify(plan.bodyParts),
        started_at: now,
        status: 'active',
        created_at: now,
        updated_at: now,
  ...
  private async insertExercise(
    ...
    await insertRecord(db, 'session_exercises', {
      id,
      user_id: this.local.userId,
  ...
  addExercise(sessionId: string, exerciseId: string) {
    return this.local.write(async (db) => {
      ...
      const order = await db.getFirstAsync<{ next: number }>(
        'SELECT coalesce(max(sort_order), -1) + 1 AS next FROM session_exercises WHERE session_id = ?',
        sessionId,
      );
  ...
  addSet(exerciseId: string) {
    return this.local.write(async (db) => {
      ...
      const previous = await db.getFirstAsync<ExerciseSet>(
        'SELECT * FROM exercise_sets WHERE session_exercise_id = ? AND deleted_at IS NULL ORDER BY set_number DESC LIMIT 1',
        exerciseId,
      );
      const library = await db.getFirstAsync<Exercise>(
        'SELECT * FROM exercises WHERE id = ?',
        exercise.exercise_id,
      );
      const next = await db.getFirstAsync<{ value: number }>(
        'SELECT coalesce(max(set_number), 0) + 1 value FROM exercise_sets WHERE session_exercise_id = ?',
        exerciseId,
      );
      ...
      await insertRecord(db, 'exercise_sets', {
        ...
        set_number: next!.value,
        tracking_type: exercise.tracking_type,
        weight_type: previous?.weight_type ?? library!.default_weight_type,
        completed: 0,
        created_at: now,
        updated_at: now,
      });
  ...
  saveSet(id: string, input: SetInput) {
    for (const value of [input.weightKg, input.reps, input.durationSeconds]) {
      if (value !== null && !Number.isFinite(value))
        throw new Error('Enter valid numbers or leave fields blank');
    }
    if (
      (input.reps !== null && (!Number.isInteger(input.reps) || input.reps <= 0)) ||
    ...
    return this.local.write(async (db) => {
      const set = await db.getFirstAsync<ExerciseSet>(
      ...
      const duration =
        exercise.tracking_type === 'duration' ? input.durationSeconds : null;
      const complete =
        (input.weightType === 'bodyweight' || weight !== null) &&
        (reps !== null || duration !== null);
      const now = this.runtime.now().toISOString();
      await db.runAsync(
        `UPDATE exercise_sets SET weight_type = ?, weight_kg = ?, reps = ?,
        duration_seconds = ?, completed = ?, updated_at = ? WHERE id = ?`,
        input.weightType,
        weight === null ? null : Math.round(weight * 1000) / 1000,
        reps,
        duration,
        Number(complete),
        now,
  ...
  private async setStatus(
    ...
    await db.runAsync(
      'UPDATE session_exercises SET status = ?, updated_at = ? WHERE id = ?',
      status,
      now,
      id,
    );
  ...
  completeExercise(id: string) {
    return this.local.write(async (db) => {
      ...
      const valid = await db.getFirstAsync(
        'SELECT id FROM exercise_sets WHERE session_exercise_id = ? AND completed = 1 AND deleted_at IS NULL',
        id,
      );
  ...
  reorderExercises(sessionId: string, ids: string[]) {
    return this.local.write(async (db) => {
      ...
      const rows = await db.getAllAsync<{ id: string }>(
        'SELECT id FROM session_exercises WHERE session_id = ? AND deleted_at IS NULL',
        sessionId,
      );
      if (
  ...
  finish(id: string) {
    return this.local.write(async (db) => {
      ...
      if (now < session.started_at)
        throw new Error('Device time is earlier than workout start');
...
export function findPrevious(
  ...
  return db.getFirstAsync<SessionExercise & { started_at: string; workout_date: string }>(
    `SELECT e.*, s.started_at, s.workout_date
    ...
    ORDER BY julianday(s.started_at) DESC, s.id DESC LIMIT 1`,
    userId,
    before,
    exerciseId,
  );
