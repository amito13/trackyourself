...
export async function readDay(db: SqlConnection, id: string): Promise<DayPlan | null> {
  const day = await db.getFirstAsync<WorkoutDay>(
    'SELECT * FROM workout_days WHERE id = ? AND deleted_at IS NULL',
    id,
  );
  if (!day) return null;
  const parts = await db.getAllAsync<{ body_part: string }>(
    `SELECT body_part FROM workout_day_body_parts
    WHERE workout_day_id = ? AND deleted_at IS NULL ORDER BY created_at, id`,
    id,
  );
  const exercises = await db.getAllAsync<Exercise>(
    `SELECT e.* FROM workout_day_exercises w
    JOIN exercises e ON e.id = w.exercise_id WHERE w.workout_day_id = ?
    AND w.deleted_at IS NULL AND e.deleted_at IS NULL ORDER BY w.sort_order, w.id`,
    id,
  );
...
export class PlanRepository {
  ...
  get() {
    return this.local.read(async (db) => {
      const plan = await db.getFirstAsync<{ id: string; name: string }>(
      ...
      const days = await db.getAllAsync<{ id: string }>(
        ...
        AND deleted_at IS NULL ORDER BY day_of_week`,
        plan.id,
      );
  ...
  save(days: PlanInput[]) {
    ...
    for (const day of days) {
      if (
        ...
        day.bodyParts.some((part) => !part.trim()) ||
        new Set(day.bodyParts).size !== day.bodyParts.length ||
        !day.exerciseIds.length ||
        new Set(day.exerciseIds).size !== day.exerciseIds.length
      ) {
    ...
    return this.local.write(async (db) => {
      ...
      for (const day of days) {
        ...
        for (const id of day.exerciseIds) {
          ...
          if (!exercise || !day.bodyParts.includes(exercise.muscle_group)) {
            throw new Error(
              'Choose cached exercises belonging to the selected body parts',
            );
          }
          groups.add(exercise.muscle_group);
        }
        if (day.bodyParts.some((part) => !groups.has(part)))
          throw new Error('Choose an exercise for every body part');
      }
      ...
      if (!existing) {
        await insertRecord(db, 'workout_plans', {
          id: planId,
          user_id: userId,
      ...
      const oldDays = await db.getAllAsync<{ id: string }>(
        'SELECT id FROM workout_days WHERE plan_id = ? AND deleted_at IS NULL',
        planId,
      );
      for (const day of oldDays) {
        for (const table of [
          ...
          const mappings = await db.getAllAsync<{ id: string }>(
            `SELECT id FROM ${table} WHERE workout_day_id = ? AND deleted_at IS NULL`,
            day.id,
          );
      ...
      for (const day of days) {
        ...
        await insertRecord(db, 'workout_days', {
          id: dayId,
          user_id: userId,
          plan_id: planId,
          day_of_week: day.dayOfWeek,
          sort_order: day.dayOfWeek - 1,
          created_at: now,
          updated_at: now,
        });
        for (const bodyPart of day.bodyParts) {
          await insertRecord(db, 'workout_day_body_parts', {
        ...
        for (const [order, id] of day.exerciseIds.entries()) {
          await insertRecord(db, 'workout_day_exercises', {
            id: this.runtime.uuid(),
            user_id: userId,
            workout_day_id: dayId,
            exercise_id: id,
      ...
      await db.runAsync(
        'UPDATE users SET onboarding_done = 1, updated_at = ? WHERE id = ?',
        now,
        userId,
      );
