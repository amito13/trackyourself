  ...
  session(id: string) {
    return this.local.read(async (db) => {
      ...
      const exercises = await db.getAllAsync<SessionExercise>(
        'SELECT * FROM session_exercises WHERE session_id = ? AND deleted_at IS NULL ORDER BY sort_order, id',
        id,
      );
      const sets = await db.getAllAsync<ExerciseSet>(
        `SELECT sets.* FROM exercise_sets sets
        ...
        ORDER BY e.sort_order, sets.set_number`,
        id,
      );
      return {
        ...session,
        ...
        completedSets: sets.filter(
          (set) =>
            set.completed === 1 &&
            exercises.some(
              (e) => e.id === set.session_exercise_id && e.status === 'completed',
            ),
        ).length,
        durationSeconds: session.completed_at
          ? Math.floor(
  ...
  compare(sessionExerciseId: string) {
    return this.local.read(async (db) => {
      const exercise = await db.getFirstAsync<
        SessionExercise & { started_at: string; workout_date: string }
      >(
        `SELECT e.*, s.started_at, s.workout_date
        FROM session_exercises e JOIN workout_sessions s ON s.id = e.session_id
        WHERE e.id = ? AND s.user_id = ? AND e.deleted_at IS NULL`,
        sessionExerciseId,
        this.local.userId,
      );
      if (!exercise) return null;
      const sets = await db.getAllAsync<ExerciseSet>(
        'SELECT * FROM exercise_sets WHERE session_exercise_id = ? AND deleted_at IS NULL ORDER BY set_number',
        exercise.id,
      );
      ...
      const previousSets = previous
        ? await db.getAllAsync<ExerciseSet>(
            'SELECT * FROM exercise_sets WHERE session_exercise_id = ? AND deleted_at IS NULL ORDER BY set_number',
            previous.id,
          )
        : [];
      return {
        ...
        previous: previous ? { ...previous, sets: previousSets } : null,
      };
    });
  }
}
